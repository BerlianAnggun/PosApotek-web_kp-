...
<body class="custom-body">
 <div class="custom-header">
    <img src="images/11.png" alt="Logo Apotek" class="w-20 h-20 ml-4" style="position:absolute;margin-left:50px;">
        ...
        <p class="custom-p">Struk Transaksi</p>
    ...
    <div class="transaction-info">
        @foreach($kode as $kd)
    <p class="left">Kode Transaksi = {{$kd->kode}}</p>
    @endforeach
    <p class="right">Nama Kasir = {{ Auth::user()->name }}</p>
</div>
    ...
    <table class="table1">
        ...
        <tbody class="custom-tbody">
            ...
            @foreach ($keranjang as $row)
                <tr class="custom-tr">
                    <td class="custom-td">{{ $no++ }}</td>
                    <td class="custom-td">{{ $row->produk->kode }}</td>
                    <td class="custom-td">{{ $row->produk->nama_produk }}</td>
                    <td class="custom-td">{{ $row->quantity }}</td>
                    <td class="custom-td">Rp.{{ $row->produk->harga }}</td>
                    <td class="custom-td">Rp.{{ $row->total }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot class="custom-tfoot">
            <tr>
                <th colspan="5" class="custom-th-footer">Total Keseluruhan</th>
                <td class="custom-td-footer">RP. {{ $total}}</td>
            </tr>
            <tr>
                <th colspan="5" class="custom-th-footer">Pembayaran</th>
                <td class="custom-td-footer">RP. {{ $jumlah_uang}}</td>
            </tr>
            <tr>
                <th colspan="5" class="custom-th-footer">Kembalian</th>
                <td class="custom-td-footer">RP. {{ $kembalian}}</td>
            </tr>
        </tfoot>
    </table>
